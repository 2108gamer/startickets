    ...
    run: async (client, interaction) => {
        interaction.message.delete().catch(console.error);
         ...
         currentQuestionMessage = await interaction.reply({
            content: 'Please provide the ID of your Tebex purchase in the chat.',
        ...
        collector.on('collect', async (msg) => {
            ...
            try {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setColor('#0099ff')
                    .addFields(
                        { name: 'Purchased on', value: `<t:${new Date(
                    data.date 
                  ).getTime() / 1000}>`, inline: false },
                        { name: 'Player', value: `${data.player.name}`, inline: false },
                        { name: 'Price', value: `${data.amount}`, inline: false },
                        { name: 'Status', value: `${data.status}`, inline: false }
                    );
                    interaction.channel.bulkDelete("2")
        .then((deleteMessages) => {
            interaction.channel.send(`> processing...`)
            .then((msg) => {
                setTimeout(() => {
                    msg.delete();
                }, 5000);
