...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        const mapIntCmds = client.applicationcommandsArray.map((v) => `\`${(v.type === 2 || v.type === 3) ? '' : '/'}${v.name}\`: ${v.description || '(Ninguna)'}`);
            ...
            embeds: [
                new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: 'Comandos de barra diagonal', value: `${mapPreCmds.join('\n')}` }
                    )
            ]
        });
